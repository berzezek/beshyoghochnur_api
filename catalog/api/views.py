    ...
    def get_queryset(self):
        ...
        if search_query:
            queryset = queryset.filter(
                ...
                relevance=(
                    Case(When(translations__name__icontains=search_query, then=Value(2)), default=Value(0), output_field=IntegerField()) +
                    Case(When(translations__description__icontains=search_query, then=Value(1)), default=Value(0), output_field=IntegerField())
                )
            ).order_by('-relevance')
